...
class BubbleShowCase(builder: BubbleShowCaseBuilder) {
    ...
    private val mDismissOnTargetDown: Boolean = builder.mDismissOnTargetDown
    ...
    private fun addTargetViewAtBackgroundDimLayout(
        ...
        if (mDismissOnTargetDown) {
            targetScreenshotView.setOnTouchListener { v, event ->
                if (event.action == MotionEvent.ACTION_DOWN) {
                    targetScreenshotView.performClick()
                }
                mBubbleShowCaseListener?.onTouch(this, event)
                true
            }
        }
